...
process.zacl <- function(data, p = 0.95, k = 0.98, theta.star = 45, epoch.seconds = 60,
      ...
      data <- postuR::check.nonwear(data, filter = F,
      	          window = nonwear.window, tol=nonwear.tol, minimum.wear.bout = minimum.wear.bout)
      ...
      ### EPOCH DATA
            ...
            min.data <-
                  ...
                  dplyr::group_by(wear.bout) %>%
                  tidyr::nest() %>%
                  dplyr::mutate(top = purrr::map(data, ~postuR:::find.top(.,p=p)),
                         min.data = purrr::map(data, ~ dplyr::group_by(.,time) %>%
                         	          	dplyr::summarise(mad = mad(x,y,z),
                                                            x = median(x),
                                                            y = median(y),
                                                            z = median(z),
                         	          		...
                         	          		.groups = "drop") %>%
                         	          	dplyr::arrange(time)),
      # add angle of inclination and rotated data
                         min.data = purrr::map2(min.data, top,
                                    ~dplyr::mutate(.x,
                                            theta = 180/pi*acos((x*.y["x"] + y*.y["y"] + z*.y["z"])/sqrt(sum(.y^2))),
                                            down0 = theta >= (theta.star))),
                         rdata = purrr::map2(min.data, top,
      ...
      min.data$cluster =  purrr::map(min.data$min.data,
       ...
       # adjust recumbent indicator using clusters
                 min.data %>%
                 dplyr::mutate(min.data = purrr::map2(min.data, cluster,
                                              ~ dplyr::mutate(.x, cluster = .y)%>%
                                     	dplyr::group_by(cluster) %>%
                                     	dplyr:: mutate(p.down = mean(down0),
